...
public class AiService {
    ...
    public Flux<ServerSentEvent<String>> streamMessage(Long roomId, Long userId, String content) {
        ...
        // 2. Redis에서 이전 대화 컨텍스트 로드 — 유저 메시지 저장 전에 조회해야 현재 메시지 중복 방지
        ...
        // 4. TOKEN 스트리밍 — 이전 대화 히스토리 + 현재 메시지, Tool Calling 포함
        ...
        // 5. TOPIC 이벤트 — 첫 메시지일 때만 채팅방 title 생성
        ...
        // 7. Fallback — AI 장애 시 ERROR 이벤트로 오류 안내 후 DONE으로 스트림 종료
        return tokenStream
