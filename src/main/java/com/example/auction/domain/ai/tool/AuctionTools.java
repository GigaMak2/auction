...
public class AuctionTools {
    ...
    public List<AuctionBidInfo> getBidsByAuctionId(Long auctionId) {
        ...
        if (results.isEmpty()) {
            throw new RuntimeException("해당 경매(ID: " + auctionId + ")의 입찰 내역이 없습니다. 입찰가나 경쟁 현황을 추측하지 마세요.");
        }
        return results;
    }
    ...
    public List<AuctionResultInfo> getRecentAuctionResults(String itemName) {
        ...
        List<AuctionResultInfo> results = aiToolRepository.findRecentAuctionResultsByItemName(itemName);
        ...
        if (results.isEmpty()) {
            ...
            throw new RuntimeException("'" + itemName + "'의 낙찰 이력이 없습니다. 시세나 낙찰가를 추측하지 마세요.");
        }
        return results;
    }
    ...
    public List<String> getSellerReviewInsights(Long sellerId, String query) {
        log.info("[Tool] getSellerReviewInsights called — sellerId={}, query={}", sellerId, query);
